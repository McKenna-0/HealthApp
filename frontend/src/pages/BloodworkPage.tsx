...
export default function BloodworkPage() {
  ...
  return (
    <>
      ...
      <PanelForm markers={markers.data ?? []} />
      ...
      {historyMarker && (history.data?.length ?? 0) < 2 && (
        <p className="muted">Log {historyMarker} in at least two panels to see a trend.</p>
      )}
      ...
      {(panels.data ?? []).map((p) => (
        <div key={p.id} className="card">
          <div className="row" style={{ justifyContent: 'space-between' }}>
            ...
            <button className="del fixed" onClick={() => deletePanel.mutate(p.id)}>
              ✕
            </button>
          </div>
            ...
            <tbody>
              {p.results.map((r) => (
                <tr key={r.id} onClick={() => setHistoryMarker(r.marker)} style={{ cursor: 'pointer' }}>
                  <td>{r.marker}</td>
                  <td style={{ color: r.flag ? '#f87171' : undefined, fontWeight: r.flag ? 700 : undefined }}>
                    {r.value} {r.unit}
                  </td>
                  <td className="muted">
      ...
      {(panels.data ?? []).length === 0 && <p className="muted">No panels logged yet.</p>}
    </>
  )
}

function PanelForm({ markers }: { markers: MarkerRef[] }) {
  const qc = useQueryClient()
  ...
  if (!open) {
    return (
      <button style={{ width: '100%', marginBottom: 12 }} onClick={() => setOpen(true)}>
        + Add panel
      </button>
    )
  }
  ...
  return (
    <div className="card">
      ...
      <div className="row">
        <button className="secondary" onClick={() => setRows([...rows, { marker: '', value: '' }])}>
          + Row
        </button>
        <button onClick={() => add.mutate()} disabled={add.isPending || !valid}>
          Save panel
        </button>
        <button className="secondary fixed" onClick={() => setOpen(false)}>
          Cancel
        </button>
      </div>
